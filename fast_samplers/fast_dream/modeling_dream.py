...
from .generate import DreamGenerationMixin, DreamGenerationConfig
...
class DreamRotaryEmbedding(nn.Module):
    def __init__(
        ...
        if config is None:
            logger.warning_once(
                "Pass a DreamConfig to DreamRotaryEmbedding for complete RoPE configuration."
            )
            self.rope_kwargs = {
                "rope_type": rope_type,
            ...
            # Support the legacy "type" key used by older RoPE configs.
...
class DreamAttention(nn.Module):
    """Multi-headed attention used by Dream."""
    ...
    def forward(
        ...
        if position_embeddings is None:
            logger.warning_once(
                "Computing RoPE embeddings inside attention because position_embeddings was not provided."
            )
...
class DreamSdpaAttention(DreamAttention):
    """Dream attention implemented with scaled dot-product attention."""
    ...
    def forward(
        ...
        if output_attentions:
            logger.warning_once(
                "DreamSdpaAttention does not support output_attentions=True; falling back to DreamAttention."
            )
        ...
        if past_key_value is not None:
            if dual_cache:
                ...
                # replace_position is identical across batch rows, use first row's indices
        ...
        # Custom CUDA attention masks require contiguous query, key, and value tensors.
...
class DreamPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Load the complete diffusion-specific generation configuration.
